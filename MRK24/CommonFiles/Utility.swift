...
enum RegEx: String {
    ...
    case username = "\\A\\w{7,10}\\z" // user Name
}
...
enum AlertMessages: String {
    ...
    case inValidUserName = "Invalid User"
...
class Validation: NSObject {
    ...
    func validate(values: (type: ValidationType, inputValue: String)...) -> Valid {
        for valueToBeChecked in values {
            ...
            case .password:
                if let tempValue = isValidString((valueToBeChecked.inputValue, .password, .emptyPSW, .emptyPSW)) {
                    return tempValue
                }
            case .username:
                if let tempValue = isValidString((valueToBeChecked.inputValue, .username, .emptyAlphabeticString, .inValidUserName)) {
                    return tempValue
                }
            }
